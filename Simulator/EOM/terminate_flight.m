...
function [terminate] = terminate_flight(t,altitude, alpha, beta, phi_theta_psi,n,sim_options)
    ...
    %%%%%%%%%%%%% PHI %%%%%%%%%%%%%%%%%%%%%
    if phi_theta_psi(1)*180/pi > departure_deg.theta_depart_deg(2)
        terminate = true;
         warning(append('Phi + departure at t = ', num2str(t),' (s),  phi = ', num2str(phi_theta_psi(1)*180/pi),' (deg)'))
    elseif phi_theta_psi(1)*180/pi < departure_deg.theta_depart_deg(1)
        terminate = true;
        warning(append('Phi - departure at t = ', num2str(t),' (s),  phi = ', num2str(phi_theta_psi(1)*180/pi),' (deg)'))
    else
        terminate = false;
